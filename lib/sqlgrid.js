...
let SQLGrid = function (sequelizeOpts, bucketOptions) {
  ...
  let {sql, files, fileChunks, chunks} = initDatabase(sequelizeOpts, conf)
    ...
    stream._read = async function () {
      ...
      if (file == null) {
        ...
        if (spec.seekStart != null) {
          startNum = Math.floor(spec.seekStart / chunkSizeBytes)
          startOffset = spec.seekStart % chunkSizeBytes
        }

        spec.seekEnd = spec.seekEnd || file.length + 1
        endNum = Math.floor((spec.seekEnd) / chunkSizeBytes)
        endOffset = (spec.seekEnd % chunkSizeBytes) + 1
