...
let SQLGrid = function (sequelizeOpts, bucketOptions) {
  if (sequelizeOpts.dialect === 'sqlite') { defaultBucketOptions.concurrency = 1 } // Disable concurrency for sqlite
  ...
  let destroyFileById = async function (id) {
    ...
    for (let pointer of filePointers) {
      ...
      let pointerList = await pointers.findAll({
        where: {
          chunk_id: chunkId
        }
      })
    ...
    let next = async function () {
      ...
      while (queue.length < conf.concurrency && queueNum <= endNum) {
        ...
        if (chunk != null) {
          ...
          let query = Promise.resolve(async function (file, num) {
